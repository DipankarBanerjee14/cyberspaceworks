...
const Loader = () => {
  return (
    <div className="relative w-[250px] aspect-square">
      {/* Ripple Boxes */}
      {[1, 2, 3, 4, 5].map((i) => {
        const inset = `${(5 - i) * 10}%`; // 40%, 30%, 20%, 10%, 0%
        const delay = `${(i - 1) * 0.2}s`; // 0s, 0.2s, 0.4s...
        const opacity = 0.2 + (i - 1) * 0.2; // 0.2, 0.4, 0.6, 0.8, 1
        return (
          <div
            key={i}
            className="absolute rounded-full border-t backdrop-blur-sm bg-[rgba(100,100,100,0.15)]"
            style={{
              inset,
              zIndex: 100 - i,
              borderColor: `rgba(100,100,100,${opacity})`,
              animation: `ripple 2s ease-in-out ${delay} infinite`,
            }}
          />
        );
      })}
      ...
      <div className="absolute inset-0 grid place-content-center p-[30%]">
        <svg
          ...
          style={{ animation: "color-change 2s ease-in-out infinite" }}
        >
